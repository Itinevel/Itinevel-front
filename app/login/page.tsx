...
import Spinner from '@/components/utils/Spinner'; // Importing the Spinner component
...
const LoginPage = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('https://itinevel-back.vercel.app/api/forgot-password', {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    <div className="lg:w-2/3 relative hidden lg:block h-screen lg:h-auto">
      <img src={travelImage.src} alt="Travel" className="absolute inset-0 w-full h-full object-cover" />
